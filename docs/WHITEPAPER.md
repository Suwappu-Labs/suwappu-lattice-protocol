...
domains as possible. As of v0.4.2 the reference implementation enforces this by
construction: each replica slot's consistent-hash candidate sequence is filtered to
regions not yet hosting a replica of that shard, falling back to region-repeating (but
still node-distinct) selection only once every available region hosts one. Selection
remains a pure function of (entity_id, shard_index, replica count, active node set), so
sender and receiver derive identical placements, and an exhaustive linear-scan tail
guarantees no replica slot is silently dropped when enough distinct nodes exist.
...
| §2.1.2 / §5.4.1.1 require replicas across distinct failure domains | **Enforced by construction as of v0.4.2** — replicas span min(r, R) distinct regions, deterministic, with graceful degradation below R regions | Resolved (was: placement never consulted `node.region`, disclosed in 0.3.0). The `check_cross_region_placement` diagnostic now passes by construction wherever ≥ 2 regions exist, and the §5.4.1.1 cross-region availability figures rest on an enforced invariant rather than an assumption. The remaining gap between model and deployment is the one §5.4.1.2 owns: region labels must actually correspond to independent failure domains, which no placement algorithm can verify. |
