...
| [`etp-protocol-revised.vp`](formal/etp-protocol-revised.vp) | Verifpal v0.27.4 | Model of the **planned** sealed-key revision (whitepaper Theorem 9, §3.3.6): receiver-generated freshness nonce, receiver-fingerprint + entity_id in the sealed key's AEAD associated data, sealed commitment reference with receiver cross-checks. Not the v1 wire format. |
| [`verifpal-run-2026-08-23-revised.md`](formal/verifpal-run-2026-08-23-revised.md) | Verifpal v0.27.4 | Runs against the revised model, **bounded, not completed** — the state space exceeds what the environment can exhaust (127.9M states explored in the longest run). Baseline 2026-08-16 results reproduced exactly first. Only failure found in the explored space: a benign verbatim-relay trace against strict sealed-key delivery-authentication, in which the receiver's own cross-checks fail closed. No claim of verification is made for the unexplored space. |
| [`../formal/lean/`](../formal/lean/) | Lean 4 (core, no Mathlib) | **Machine-checked.** Corridor 7-of-9 quorum safety + liveness, constant-size commitment **and sealed-lattice-key** invariants, §6.4 bandwidth break-even, erasure k-of-n threshold consequences, access-policy algebra, 2/3-supermajority BFT bounds, both §2.1.1 test vectors recomputed in-kernel over GF(2⁸), the §3.3.5 ramp-scheme entropy ledger (leakage/candidate-count arithmetic and the blinded-ramp share-size trade), the §3.3.1 concrete-security integers (BHT/birthday/Grover exponents, nonce margin, backoff law), and the §5.4.1.1 correlated-failure identities and worked figures. 90 audited theorems. Gated in CI by `.github/workflows/formal.yml`. |
