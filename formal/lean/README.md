...
| `bht_exponent` / `bht_bracket` / `collision_below_preimage` / `grover_square` | `Ltp/ConcreteSecurity.lean` | §3.3.1's post-quantum table integers: ⌊256/3⌋ = 85 with the ~85.3 bracketing (the division review 001 found misstated), 256/2 = 128, collision strictly below preimage, and Grover's 2¹²⁸·2¹²⁸ = 2²⁵⁶ accounting |
| `nonce_birthday_margin` / `backoff_doubles` / `mars_commit_upload_seconds` | `Ltp/ConcreteSecurity.lean` | The §2.1.1 nonce-collision margin (q² = 2⁶⁴ < 2⁹⁷ at q = 2³² shards), the §2.3.3 backoff doubling law, and Appendix A's 48,000 s (≈ 13.4 h) Earth-upload figure |
| `replica_complement` / `replica_two_forms` | `Ltp/Availability.lean` | §5.4.1.1's correlated-failure laws as general polynomial identities over scaled integers: (1−p_replica) = (1−p_d)(1−p_n), and the section's two expressions for p_replica agree (instantiating b := p_nʳ gives the same-region worst-case form) |
| `cross_region_bracket` / `same_region_worst` / `colocation_penalty` | `Ltp/Availability.lean` | The review-002-verified worked figures decided in-kernel — 0.0595, 595³ = 210,644,875 with the ≈ 2.1×10⁻⁴ bracket, ≈ 0.99979, ≈ 0.01012 — and the sharpened claim that same-region colocation is ≥ 48× worse, the arithmetic behind `minimum_regions ≥ 3` |
